...
Page({
  ...
  async fetchInstitutionList() {
    ...
    console.log(res)
    this.data.day = res.data.data.filter((item) => { item.file_type === '21'})
    this.data.week = res.data.data.filter((item) => { item.file_type === '22'})
    this.data.month = res.data.data.filter((item) => { item.file_type === '23'})
  },
